...
class HybridClassifier {
  ...
  constructor(serverConfig, providerRegistry) {
    ...
    const modelId =
      serverConfig.classifier?.llmClassifierModel || "deepseek-v4-flash";
    ...
    if (this.classifierProvider) {
      console.log(
        `[classifier] LLM fallback model: ${modelId} (${this.classifierModel.provider})`
      );
    } else {
      console.warn(
  ...
  async classify(requestBody) {
    ...
    // If the fused result was dragged below threshold by zero-confidence
    // signals from other classifiers, but a single high-confidence signal
    // already dominates and agrees with the fusion, skip the LLM fallback.
    const hasHighConfSignal = [ruleResult, embeddingResult, profileResult].some(
      (r) => r.confidence >= 0.8
    );
    const fusionAgrees = [ruleResult, embeddingResult, profileResult].filter(
      (r) =>
        r.confidence >= 0.8 &&
        r.taskType === fused.taskType &&
        r.complexity === fused.complexity &&
        r.costSensitivity === fused.costSensitivity
    );
    if (hasHighConfSignal && fusionAgrees.length > 0) {
