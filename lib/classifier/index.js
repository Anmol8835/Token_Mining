...
class HybridClassifier {
   ...
   */
  _classifierCallCost() {
    const usage = this.lastClassifierUsage;
    this.lastClassifierUsage = null;
    if (!usage || !this.classifierModel?.cost) return null;

    // Provider-agnostic token field names.
    const input =
    ...
    const costUsd =
      Math.round(
        ((input * this.classifierModel.cost.inputPer1M) / 1e6 +
          (output * this.classifierModel.cost.outputPer1M) / 1e6) *
          1e6
      ) / 1e6;
