...
const { normalizeUsage } = require("../converter");
...
class HybridClassifier {
  ...
  constructor(serverConfig, providerRegistry) {
    ...
    this.lastClassifierUsage = null; // set by _doChat
    ...
    const modelId =
      serverConfig.classifier?.llmClassifierModel || "deepseek-flash";
  ...
  async classify(requestBody) {
    // ------------------------------------------------------------
    // TEMPORARY (2026-09-24): classifier.mode "llm" runs ONLY the
    // LLM classifier. Rules/embedding/profile, fusion, the high-
    // confidence shortcut, and the classification cache are all
    // bypassed — every request gets a fresh LLM classification.
    // RESTORE: set config/server.json classifier.mode to "auto"
    // (this block is dormant unless mode === "llm").
    // ------------------------------------------------------------
    if (this.config?.classifier?.mode === "llm") {
      if (!this.classifierProvider || !this.classifierModel) {
        throw new Error(
          "[classifier] llm mode requested but no LLM classifier provider available"
        );
      }
      const llmResult = await classifyWithLLMFallback(
        requestBody,
        this.classifierProvider,
        this.classifierModel,
        this._doChat.bind(this)
      );
      const cost = this._classifierCallCost();
      llmResult.metadata = llmResult.metadata || {};
      llmResult.metadata.llmOnly = true;
      if (cost) llmResult.metadata.classifierCost = cost;
      return llmResult;
    }
    ...
    if (this.classifierProvider && this.classifierModel) {
      const attachClassifierCost = (result) => {
        const cost = this._classifierCallCost();
        if (cost) {
          result.metadata = result.metadata || {};
          result.metadata.classifierCost = cost;
        }
        return result;
      };
      ...
      try {
        ...
        return attachClassifierCost(refined);
      } catch (err) {
        console.warn(
        ...
        this.cache.set(requestBody, fused);
        // A failed call can still have billed usage (e.g. truncated).
        return attachClassifierCost(fused);
      }
    }
   ...
   * Captures the raw usage so the orchestrator can cost the
   * classifier call for metrics.
   */
  async _doChat(provider, nativePayload) {
    ...
    this.lastClassifierUsage = resp.data?.usage || null;
    return resp.data;
  }

  /**
   * Price the most recent classifier call from its recorded usage.
   * Resets the capture so a stale response is never double-counted.
   */
  _classifierCallCost() {
    const rawUsage = this.lastClassifierUsage;
    this.lastClassifierUsage = null;
    if (!rawUsage || !this.classifierModel?.cost) return null;

    // Normalize provider-agnostic usage shapes (OpenAI-style
    // prompt_tokens/prompt_cache_hit_tokens vs Anthropic-style
    // input_tokens/cache_read_input_tokens) the same way real
    // billing does, so cache hits aren't priced at the full rate.
    ...
    const cost = this.classifierModel.cost;
    const inputCost = input * (cost.inputPer1M / 1e6);
    const outputCost = output * (cost.outputPer1M / 1e6);
    const readCost =
      read * ((cost.cacheReadInputPer1M ?? cost.cacheHitInputPer1M ?? cost.inputPer1M) / 1e6);
    const writeCost = write * ((cost.cacheWriteInputPer1M ?? 0) / 1e6);

    const costUsd = Math.round((inputCost + outputCost + readCost + writeCost) * 1e6) / 1e6;

    return {
      model: this.classifierModel.id,
