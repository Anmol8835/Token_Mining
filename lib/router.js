...
/**
 * Load the data-driven router policy. Missing/corrupt file => empty
 * rules => the router behaves exactly as before the feature existed.
 */
 ...
 * @param {string[]} excludedIds - Model IDs disabled by dashboard prefs (auto-routing only)
 * @returns {object} { model, reason } - Selected model config and routing reason
 */
function selectModel(classification, registry, routerConfig, explicitModel, routingMode, excludedIds = []) {
  const {
    costWeight = 0.3,
    qualityWeight = 0.7,
  ...
  // Dashboard-disabled models drop out of the candidate pool. If the
  // exclusions would empty it entirely (user disabled every model the
  // request could use), ignore them rather than fail the request.
  let candidates = registry.filterModels(
    (model) => !excludedIds.includes(model.id) && capabilityFilter(model)
  );
  if (candidates.length === 0 && excludedIds.length > 0) {
    candidates = registry.filterModels(capabilityFilter);
  }

  // --- Router policy filter (data-driven cost-tier floor) ---
  // config/router-policy.json is emitted by benchmark/fit-policy.js.
  // Matching rules drop candidates whose cost tier is below the
  // rule's minimum. Missing file => zero behavior change; if the
  // filter would empty the list, routing proceeds unfiltered.
  {
    const policy = loadRouterPolicy();
    if (policy.rules.length > 0 && candidates.length > 0) {
      ...
      for (const rule of policy.rules) {
        ...
        for (const [key, want] of Object.entries(cond)) {
          const got =
            ...
            : classification[key];
          if (got !== want) { matched = false; break; }
        }
        if (matched && (minTier === null || TIER_RANK[rule.minCostTier] > TIER_RANK[minTier])) {
          minTier = rule.minCostTier;
        }
      }
      if (minTier) {
        ...
        if (kept.length > 0) {
          candidates = kept;
          console.log(
            `  policy: ${JSON.stringify(classification.primary_task)}/${classification.complexity} ` +
            `→ min tier ${minTier} (${candidates.length} candidates kept)`
          );
        }
      }
    }
...
module.exports = { selectModel, loadRouterPolicy };
