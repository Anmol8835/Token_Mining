...
class GeminiProvider extends BaseProvider {
  ...
  async streamResponse(nativeStream, res, modelName) {
    ...
    // Gemini reports usageMetadata on the final chunk.
    ...
    return new Promise((resolve) => {
        ...
        for (const line of lines) {
          ...
          for (const item of candidates) {
            if (item.usageMetadata) {
              usage.input_tokens = item.usageMetadata.promptTokenCount || 0;
              usage.output_tokens = item.usageMetadata.candidatesTokenCount || 0;
            }
      ...
      nativeStream.on("end", () => {
        // Process the trailing buffer — usageMetadata often arrives
        // in the last chunk without a trailing newline.
