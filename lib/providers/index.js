...
class ProviderRegistry {
  constructor(modelsConfig, serverConfig) {
    ...
    this.providers = new Map();     // modelId -> BaseProvider instance
    this.serverConfig = serverConfig;

    // One provider instance PER MODEL, not per provider name: each
    // instance carries its model's config (output caps, cache
    // minimums, pricing). A shared per-provider instance would read
    // the FIRST model's config for every request — e.g. all Claude
    // models clamped to Haiku's 8k output cap (2026-09-23 fix).
    for (const model of modelsConfig.models) {
      ...
      const instance = new ProviderClass(model, this.serverConfig);
      try {
        instance.validate();
        this.providers.set(model.id, instance);
      } catch (err) {
  ...
  getProvider(modelId) {
    ...
    return this.providers.get(model.id) || null;
  }

  getProviderByName(providerName) {
    for (const [id, model] of this.models) {
      if (model.provider === providerName && this.providers.has(id)) {
  ...
  filterModels(predicate) {
    return Array.from(this.models.values()).filter((m) => {
      ...
      if (!this.providers.has(m.id)) return false;
      return predicate(m);
    });
  }
